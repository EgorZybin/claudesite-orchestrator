...
def patch_page_metadata(
    ...
    """Точечная правка полей страницы.

    Логика выбора что менять: ``field in body.model_fields_set`` — клиент
    ЯВНО прислал поле в JSON. Это позволяет различать «поле не прислано»
    (skip) vs «прислано как null» (обнулить колонку — снять nav_label,
    очистить hero_image и т.д.).
    ...
    if "seo_description" in sent:
        ...
        if body.seo_description is None:
    ...
    if "modules_used" in sent and body.modules_used is not None:
...
def generate_site(
    ...
    job = jobs_repo.create(
        ...
        input_params={
            ...
            "has_design_tokens": body.design_tokens is not None,
        },
    )
    session.commit()
...
@router.get("/{site_slug}/domains")
def list_domains(
    site_slug: str,
    session: Session = Depends(readwrite_session),
) -> list[dict[str, object]]:
    """Список доменов, привязанных к сайту."""
    from app.db.repos.site_domains import SiteDomainRepository

    site = _resolve_site_or_404(session, site_slug)
    rows = SiteDomainRepository(session).list_for_site(site.id)
    return [
        {
            "id": r.id,
            "host": r.host,
...
@router.post("/{site_slug}/domains", status_code=201)
def add_domain(
    site_slug: str,
    body: DomainAdd,
    session: Session = Depends(readwrite_session),
) -> dict[str, object]:
    """Прикрепить домен к сайту (запись в БД).

    После этого нужно: (1) DNS A-record на сервер, (2) Apache vhost +
    Let's Encrypt через `scripts/provision-domain.sh <host> <slug>`. Сразу
    же `SiteHostResolverMiddleware` начинает резолвить запросы по этому
    Host header → render через `/public/{slug}/...`.
    """
    from sqlalchemy.exc import IntegrityError

    from app.db.models.site_domain import SiteDomain
    from app.db.repos.site_domains import normalize_host

    site = _resolve_site_or_404(session, site_slug)
    host = normalize_host(body.host)
    if not host:
        raise HTTPException(status_code=400, detail=f"invalid host: {body.host!r}")
    ...
    try:
        ...
        session.rollback()
        raise HTTPException(status_code=409, detail=f"host already taken: {host}") from e

    logger.info("domain_added site=%s host=%s id=%s", site_slug, host, row.id)
    return {
        "id": row.id,
        "host": row.host,
        "site_slug": site_slug,
...
@router.delete("/{site_slug}/domains/{domain_id}")
def remove_domain(
    site_slug: str,
    domain_id: int,
    session: Session = Depends(readwrite_session),
) -> dict[str, object]:
    """Удалить привязку домена. Apache vhost при этом НЕ удаляется автоматически."""
    from app.db.repos.site_domains import SiteDomainRepository

    site = _resolve_site_or_404(session, site_slug)
    repo = SiteDomainRepository(session)
    row = repo.get_by_id(domain_id)
    if row is None or row.site_id != site.id:
        raise HTTPException(status_code=404, detail=f"domain not found: id={domain_id}")
    host = row.host
    session.delete(row)
    session.commit()
    logger.info("domain_removed site=%s host=%s id=%s", site_slug, host, domain_id)
    return {"ok": True, "id": domain_id, "host": host}
